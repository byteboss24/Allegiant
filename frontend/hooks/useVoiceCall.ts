...
interface UseVoiceCallProps {
  onTranscription?: (text: string) => void; // Optional callback for transcriptions
...
export function useVoiceCall({ onTranscription, onError }: UseVoiceCallProps = {}) {
  ...
  const reportError = useCallback((message: string) => {
    ...
    if (onError) {
      ...
      alert(message); // Default error reporting if no callback provided
    }
  }, [onError]);
        ...
        if (websocketRef.current?.binaryType === "blob" || websocketRef.current?.binaryType === "arraybuffer") {
            ...
            reader.onload = () => {
                ...
                if (base64Audio) {
                    ...
                    event: 'media', // Ensure event type matches backend expectation
                    media: {
                        payload: base64Audio
                    }
                    }));
  ...
  const initializeAudio = useCallback(async () => {
    try {
      ...
      try {
        // Adjust the path if your worklet file is located elsewhere
      ...
      // Do not connect workletNode to destination unless needed for other purposes
  ...
  const playNextAudio = useCallback(() => {
    if (isPlayingRef.current || audioQueueRef.current.length === 0 || !isSpeakerOn) {
      return; // Don't play if already playing, queue is empty, or speaker is off
    }
    ...
    if (audio) {
      ...
      audio.onended = () => {
        ...
        playNextAudio(); // Play the next item
      };
      audio.onerror = (e) => {
        console.error('Error playing audio:', e);
        URL.revokeObjectURL(audio.src); // Clean up blob URL on error
        isPlayingRef.current = false;
        playNextAudio(); // Attempt to play next item even if current one failed
      };
  ...
  const initializeWebSocket = useCallback(() => {
    ...
    // Optional: Set binary type if backend supports binary frames
    ...
    websocketRef.current.onopen = () => {
      ...
      const startMessage = {
        event: 'start',
        start: {
          // streamSid should ideally be generated server-side or passed
      ...
      if (event.data instanceof Blob) {
        ...
        try {
          ...
          if (data.event === 'media' && data.media && data.media.track === 'outbound') {
            // Potentially handle outbound audio confirmation or metadata
          } else if (data.event === 'transcription') { // Adjust based on actual backend event structure
             if (onTranscription) {
    ...
    setIsMuted((prevMuted) => {
        ...
        if (micStreamRef.current) {
            micStreamRef.current.getAudioTracks().forEach((track) => {
                track.enabled = !nextMuted; // Track is enabled if *not* muted
            });
        }
