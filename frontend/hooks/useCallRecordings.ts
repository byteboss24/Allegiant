...
export function useCallRecordings(initialItemsPerPage = 10) {
  ...
  const audioRef = useRef<HTMLAudioElement>(null); // Keep audioRef for potential external control if needed
  ...
  useEffect(() => {
    const currentAudioUrl = audioUrl; // Capture the current URL
    return () => {
      if (currentAudioUrl) {
  ...
  const handleSelectCall = useCallback(async (call: CallRecordingItem) => {
    if (selectedCall?.id === call.id) { // Don't re-fetch if already selected
      if (audioRef.current) audioRef.current.play(); // Just play
      return;
    }
  ...
  }, [selectedCall, audioUrl, toast]); // Include dependencies
  ...
  const confirmMultiDelete = useCallback(async () => {
    ...
    const idsToDelete = [...selectedIdsForMultiDelete]; // Copy ids
    try {
  ...
  return {
    ...
    audioUrl,
    audioLoading,
    audioRef, // Expose ref if CallPlayer needs direct access
    selectedIds: selectedIdsForMultiDelete, // Rename for clarity in component
    showDeleteDialog,
    ...
    confirmMultiDelete,
    setShowDeleteDialog, // Allow component to close dialog
    setShowMultiDeleteDialog, // Allow component to close dialog
  };
} 