...
class MySQLService:
    ...
    async def get_invoices_to_process(self):
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute("SELECT * FROM invoices WHERE status != 'completed' and status != 'calling' and status != 'sms' and status != 'reserve' and status != 'seek' and status != 'voice_message' and is_deleted = 0 LIMIT 10")
